...
wss.on('connection', function (ws) {
    ...
    var minicap = net.connect({
        port: 1717
    });
    var minitouch = net.connect({
        port: 1111
    });

    minicap.on('error', function () {
        console.error('Be sure to run `adb forward tcp:1717 localabstract:minicap`');
        process.exit(1)
    });
    minitouch.on('error', function () {
    ...
    minicap.on('readable', tryRead);
    minitouch.on('data', function(data){
